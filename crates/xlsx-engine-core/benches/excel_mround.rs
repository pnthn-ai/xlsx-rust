...
//!
//! Compares the first-draft snap-both-args then snap-quotient path
//! (`excel_mround_naive` / slice) with the production kernel (unit-multiple
//! `ROUND`, safe-integer path, cheap 15-digit snap-to-half).
//!
//! ```text
...
fn main() {
    println!("MROUND kernel bench (15-digit snap-then-half vs int-path + leftover snap)");
    println!(
        "{:<42} {:>12} {:>12} {:>8}",
...

    {
        ...
        assert_eq!(out_a, out_b, "slice MROUND ints mismatch");
    }
...

    {
        let naive = time_it(ITERS, || {
            let mut acc = 0.0;
            for n in &dec {
                acc += excel_mround_naive(black_box(*n), black_box(0.05)).unwrap_or(0.0);
            }
            black_box(acc);
        });
        let fast = time_it(ITERS, || {
            let mut acc = 0.0;
            for n in &dec {
                acc += excel_mround(black_box(*n), black_box(0.05)).unwrap_or(0.0);
            }
            black_box(acc);
        });
        row("200k scalar MROUND(n, 0.05) dec", naive, fast);
    }

    {
        ...
        let mut out_b = vec![0.0; N];
        let naive = time_it(ITERS, || {
            excel_mround_slice_naive(black_box(&dec), black_box(0.05), black_box(&mut out_a));
        });
        let fast = time_it(ITERS, || {
            excel_mround_slice(black_box(&dec), black_box(0.05), black_box(&mut out_b));
        });
        row("200k slice MROUND(n, 0.05) dec", naive, fast);
    }

    {
        ...
        row("200k scalar MROUND(n, 0.1) near-mult", naive, fast);
...

    {
        ...
        let naive = time_it(ITERS, || {
            excel_mround_slice_naive(black_box(&mult), black_box(0.1), black_box(&mut out));
        });
        let fast = time_it(ITERS, || {
            excel_mround_slice(black_box(&mult), black_box(0.1), black_box(&mut out));
        });
        row("200k slice MROUND(n, 0.1) near-mult", naive, fast);
    }

    {
        let naive = time_it(ITERS * 4, || {
            ...
            for n in ns.iter().step_by(8) {
                if excel_mround_naive(black_box(n.abs().max(1.0)), black_box(-2.0)).is_err() {
                    acc += 1;
                }
                if excel_mround_naive(black_box(*n), black_box(0.0)).is_ok() {
                    acc += 1;
                }
            }
            black_box(acc);
        ...
        let fast = time_it(ITERS * 4, || {
            let mut acc = 0u32;
            for n in ns.iter().step_by(8) {
                if excel_mround(black_box(n.abs().max(1.0)), black_box(-2.0)).is_err() {
                    acc += 1;
                }
                if excel_mround(black_box(*n), black_box(0.0)).is_ok() {
                    acc += 1;
                }
            }
            black_box(acc);
