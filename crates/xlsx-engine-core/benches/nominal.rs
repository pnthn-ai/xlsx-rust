//! Before/after microbench for Excel `NOMINAL`.
//!
//! Compares the textbook `powf` baseline (`excel_nominal_naive`) with the
//! production kernel (`excel_nominal`: n=1 identity, n=2 rationalized
//! `2e/(√(1+e)+1)`, `expm1(ln1p(e)/n)`).
//!
//! ```text
//! cargo bench -p xlsx-engine-core --bench nominal
//! ```

use std::hint::black_box;
use std::time::{Duration, Instant};
use xlsx_engine_core::{excel_nominal, excel_nominal_naive};
...
fn cases() -> Vec<Case> {
    ...
    vec![
        ...
        Case {
            name: "Microsoft NOMINAL(0.053543, 4)",
            rates: vec![0.053543; 10_000],
            npery: 4.0,
            iters: 400,
        },
    ]
...
fn main() {
    println!("NOMINAL kernel bench (powf baseline vs specialized)");
    println!(
        "{:<42} {:>12} {:>12} {:>8}",
    ...
    for c in cases() {
        ...
        let fast = time_it(c.iters, || {
            black_box(fold(excel_nominal, black_box(&c.rates), black_box(c.npery)));
        });
        ...
        for &r in c.rates.iter().step_by((c.rates.len() / 16).max(1)) {
            let a = excel_nominal_naive(r, c.npery);
            let b = excel_nominal(r, c.npery);
            match (a, b) {
                (Ok(a), Ok(b)) => {
