//! Before/after microbench for Excel `FLOOR` / `CEILING`.
//!
//! Compares the IEEE `significance * (n/significance).floor()` baseline
//! (`excel_floor_naive` / slice) with the production kernel (safe-integer
//! path + 15-digit multiple snap).
//!
//! ```text
//! cargo bench -p xlsx-engine-core --bench floor_ceiling
//! ```
...
fn main() {
    println!("FLOOR/CEILING kernel bench (IEEE multiply vs integer path + 15-digit snap)");
    println!(
        "{:<42} {:>12} {:>12} {:>8}",
    ...
    // Slice integer FLOOR
    {
        ...
        assert_eq!(out_a, out_b, "slice FLOOR mismatch");
    }

    // Slice integer CEILING
    {
        let mut out_a = vec![0.0; N];
        let mut out_b = vec![0.0; N];
        let naive = time_it(ITERS, || {
        ...
        assert_eq!(out_a, out_b, "slice CEILING mismatch");
    }

    // Decimal significance — optimized must stay correct; naive may drift.
    {
        let naive = time_it(ITERS, || {
            let mut acc = 0.0;
            for n in &dec {
...

    {
        ...
        let naive = time_it(ITERS, || {
            excel_ceiling_slice_naive(black_box(&dec), black_box(0.1), black_box(&mut out));
        });
        let fast = time_it(ITERS, || {
            excel_ceiling_slice(black_box(&dec), black_box(0.1), black_box(&mut out));
        });
        row("200k slice CEILING(n, 0.1) dec", naive, fast);
        // Production snap: 1.2-like values stay on the 0.1 grid.
        assert_eq!(excel_ceiling(1.2, 0.1).unwrap(), 1.2);
    }

    // Mixed-sign / zero-sig error path (cheap; proves the checks stay in the hot fn).
    {
