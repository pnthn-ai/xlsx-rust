//! Microbenches for Excel `EDATE`.
//!
//! Compares the production helper (`edate_serial`, which reuses the O(1)
...
use xlsx_engine_core::dates::{
    date_serial, days_in_month, edate_serial, is_excel_leap, normalize_year_month, serial_to_ymd,
};
...
fn edate_serial_walk(
    ...
    let (year, month, day) = serial_to_ymd_walk(start, system)?;
    ...
    let s1900 = ymd_to_serial_1900_walk(year, month, day)?;
    match system {
...
fn main() {
    ...
    let y2k = date_serial(2000, 1, 15, SYSTEM).unwrap();
    let late = date_serial(9999, 11, 15, SYSTEM).unwrap();
    let mid = date_serial(2011, 1, 15, SYSTEM).unwrap();
    ...
    for &(start, months) in &[
        ...
        let a = edate_serial(start, months, SYSTEM).unwrap();
        let b = edate_serial_walk(start, months, SYSTEM).unwrap();
    ...
    for _ in 0..ITERS {
        for &(start, months) in cases {
            run_pair(edate_serial_walk, start, months);
        }
    }
    ...
    for _ in 0..ITERS {
        for &(start, months) in cases {
            run_pair(edate_serial, start, months);
        }
    }
    ...
    for _ in 0..ITERS {
        black_box(edate_serial(black_box(60.0), black_box(0.0), SYSTEM).unwrap());
    }
    let serial60_ns = t2.elapsed().as_nanos() / ITERS as u128;

    let t3 = std::time::Instant::now();
    for _ in 0..ITERS {
        black_box(edate_serial(black_box(modern), black_box(1.0), SYSTEM).unwrap());
    }
    ...
    println!("edate microbench  iters={ITERS}  cases={}", cases.len());
    println!("  walk-ref mixed        {walk_ns} ns/call");
    println!("  edate_serial mixed    {fast_ns} ns/call");
    println!("  edate_serial(60,0)    {serial60_ns} ns/call");
    println!("  edate_serial(2024-01-31,1) {modern_ns} ns/call");
