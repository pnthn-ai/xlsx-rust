//! Before/after microbench for Excel `WEEKNUM`.
//!
//! Compares the calendar-walk baseline (`excel_weeknum_naive`: YMD + day walk
//! from January 1) with the production kernel (`excel_weeknum`: O(1) year +
//! serial arithmetic).
//!
//! ```text
//! cargo bench -p xlsx-engine-core --bench weeknum
//! ```

use std::hint::black_box;
use std::time::{Duration, Instant};
use xlsx_engine_core::{excel_weeknum, excel_weeknum_naive};
...
fn cases() -> Vec<Case> {
    ...
    let mixed: Vec<f64> = (0..10_000)
        ...
        .collect();
    let types_all = vec![1, 2, 11, 12, 13, 14, 15, 16, 17, 21];
    vec![
        Case {
        ...
        Case {
            name: "10k strided serials × type 21 ISO",
            serials: (0..10_000)
                .map(|i| (i * 293) as f64 % 2_958_466.0)
                .collect(),
            return_types: vec![21],
...
fn run_kernel(f: fn(f64, i32, DateSystem) -> Result<f64, xlsx_types::ExcelError>, c: &Case) {
    for &s in &c.serials {
...
fn main() {
    println!("WEEKNUM kernel bench (day-walk vs O(1) serial arithmetic)");
    println!(
        "{:<42} {:>12} {:>12} {:>8}",
    ...
    println!("{}", "-".repeat(78));
    for c in cases() {
        let naive = time_it(c.iters, || run_kernel(excel_weeknum_naive, &c));
        let fast = time_it(c.iters, || run_kernel(excel_weeknum, &c));
        ...
        for &s in &c.serials {
            for &rt in &c.return_types {
                let a = excel_weeknum_naive(s, rt, c.system);
                let b = excel_weeknum(s, rt, c.system);
