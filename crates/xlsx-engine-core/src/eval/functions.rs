...
//! Financial TVM: `PMT` / `CUMIPMT` (`xlsx_types::excel_pmt` / `excel_cumipmt`).
//! `PV` / `FV` / `NPER` / `IPMT` are later workstreams.
...
use xlsx_types::{
    count_matches, excel_ceiling, excel_ceiling_math, excel_cumipmt, excel_floor, excel_floor_math,
...
pub(crate) fn dispatch(
    ...
    match name.to_ascii_uppercase().as_str() {
        ...
        // Financial (TVM). PV / FV / NPER / IPMT are later workstreams.
