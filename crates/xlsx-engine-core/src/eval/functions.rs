...
//! Financial TVM starts with `PMT` / `NOMINAL` (`xlsx_types::excel_pmt` /
//! `excel_nominal`); `PV`/`FV`/`NPER`/`EFFECT` are later workstreams.
...
use xlsx_types::{
    count_matches, excel_ceiling, excel_ceiling_math, excel_floor, excel_floor_math, excel_nominal,
...
pub(crate) fn dispatch(
    ...
    match name.to_ascii_uppercase().as_str() {
        ...
        // Financial (TVM). PV / FV / NPER / EFFECT are later workstreams.
