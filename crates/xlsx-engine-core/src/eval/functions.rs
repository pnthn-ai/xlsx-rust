...
//! Financial TVM starts with `PMT` / `EFFECT` (`xlsx_types::excel_pmt` /
//! `excel_effect`); `PV`/`FV`/`NPER`/`NOMINAL` are later workstreams.
...
use xlsx_types::{
    count_matches, excel_ceiling, excel_ceiling_math, excel_effect, excel_floor, excel_floor_math,
...
pub(crate) fn dispatch(
    ...
    match name.to_ascii_uppercase().as_str() {
        ...
        // Financial (TVM). PV / FV / NPER / NOMINAL are later workstreams.
