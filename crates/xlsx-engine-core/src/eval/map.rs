...
use super::makearray::{names_eq, resolve_lambda_any, Local, MAX_COLS, MAX_ROWS};
...
pub(crate) fn apply_naive(
    ...
    for p in params {
        ctx.locals.push(Local::provided(p.clone(), EMPTY));
    }
    let mut grid = Vec::with_capacity(rows);
    for r in 0..rows {
        let mut row = Vec::with_capacity(cols);
...
mod tests {
    ...
    fn classify_x_times_2() {
        ...
        assert_eq!(
            classify(&parse("X*2").unwrap(), &["x".into()]),
            Some(times2())
        );
    }
    ...
    fn identity_wraps_scalar() {
        ...
        assert_eq!(fill_fast(&[n(7.0)], &plan), arr(vec![vec![n(7.0)]]));
    }
    ...
    fn union_pads_na() {
        ...
        assert_eq!(
            got,
            arr(vec![vec![n(2.0), NA.clone()], vec![NA.clone(), NA.clone()],])
        );
        assert_eq!(
            got,
            fill_naive(
