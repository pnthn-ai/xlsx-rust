...
//! cell-content width, the same cap as `CONCAT` / `REPT`. Microsoft’s TEXTJOIN
//! page says “32767 characters”; that is this cell-width limit, **not** this
//! crate’s Compatibility Version 2 `LEN` (Unicode scalars).
//!
//! - `TEXTJOIN("", TRUE, REPT("a", 32767))` is allowed; one extra ASCII unit
//!   is `#VALUE!`.
//! - Delimiters count toward the cap: `TEXTJOIN(",", TRUE, REPT("a", 32766),
//!   "b")` is `#VALUE!` (32766 + 1 comma + 1).
...
//!   `#VALUE!` overflow — this kernel does **not** read fixture goldens.
//!
//! Production path: walk occupied cells when `ignore_empty` is TRUE and the
//! rectangle is much larger than the sheet store (empties add nothing, so
//! skipping missing keys is equivalent). When `ignore_empty` is FALSE, blanks
//! consume delimiter slots and the walk must visit every address.
//!
//! `CONCAT` and `CONCATENATE` are out of scope.
...
pub enum TextJoinWalk {
    ...
    Dense,
    /// Production: dense walk, or occupied-cell gather when `ignore_empty`
    /// lets us skip blanks on a large sparse range.
    Auto,
}
...
/// O(area × lookup). Use occupied only when `ignore_empty` is TRUE — empty
/// cells add nothing, so skipping missing keys is equivalent. FALSE must
/// visit every address (blanks consume a delimiter slot). `Dense` forces
/// the rectangle walk so the bench can show the gather win on sparse columns.
fn prefer_occupied(
    range: &RangeRef,
    occupied: u64,
    walk: TextJoinWalk,
...
/// Flatten a delimiter value into cycling strings (arrays row-major).
/// Shared with `seed-compliant` so the empty-array → `""` rule stays one
/// implementation. Does not read fixture goldens.
...
mod tests {
    ...
    #[test]
    fn delimiter_counts_toward_cap() {
        let mut b = TextJoinBuilder::new(vec![",".into()]);
        b.push(&"x".repeat(32766), true).unwrap();
        assert!(b.push("y", true).is_err());
    }
    ...
    fn occupied_walk_is_row_major_not_a1_key_order() {
        // A1-string BTree order is A1, A10, A2 — not row-major.
        ...
        // area = 20, occupied = 6 → Auto uses occupied gather when ignore TRUE.
    ...
    fn sparse_far_cells_skip_gaps_when_ignore() {
        ...
        for v in eval_all(&wb, "=TEXTJOIN(\",\",FALSE,A1:A100)") {
            assert_eq!(v, ExcelValue::Text(keep.clone()));
        }
    }

    #[test]
    fn prefer_occupied_requires_ignore_empty() {
