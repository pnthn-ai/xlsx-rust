...
use super::makearray::{fn_key, names_eq, resolve_lambda_n, Local, MAX_COLS};
...
impl Acc {
    ...
    fn finish(self, op: &ColOp) -> ExcelValue {
        match op {
            ...
            ColOp::Product => ExcelValue::Number(if self.count == 0 { 0.0 } else { self.product }),
            ColOp::Average => {
                if self.count == 0 {
...
fn apply_ast(
    ...
    ctx.locals.push(Local::provided(param.to_string(), EMPTY));
    let mut out = Vec::with_capacity(ncols);
    for c in 0..ncols {
        ctx.locals[base].value = column_arg(rows, c);
...
mod tests {
    ...
    fn classify_sum_of_param() {
        ...
        assert_eq!(
            classify(&parse("7").unwrap(), "c"),
            Some(ColOp::Const(n(7.0)))
        );
    }
    ...
    fn body_error_stays_in_cell() {
        let v = eval_formula_in(&Workbook::default(), "=BYCOL({1,0,2},LAMBDA(c,1/c))").unwrap();
        assert_eq!(
            v,
