...
use super::makearray::{lookup_binding, names_eq, strip_xlpm};
...
pub fn is_bound(locals: &[(String, ExcelValue)], name: &str) -> bool {
...
pub(crate) fn apply(
    ...
    let names: Vec<String> = ctx.locals.iter().map(|(n, _)| n.clone()).collect();
    let values: Vec<ExcelValue> = ctx.locals.iter().map(|(_, v)| v.clone()).collect();
