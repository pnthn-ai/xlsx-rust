...
use super::makearray::{names_eq, FastOp};
...
fn scan_walk(initial: Option<&ExcelValue>, grid: &[Vec<ExcelValue>], body: &FastScan) -> ExcelValue {
    if grid.is_empty() || grid[0].is_empty() {
        return ExcelValue::Error(ExcelError::Calc);
    }
    let rows = grid.len();
...
pub(crate) fn apply_naive(
    ...
    ctx.locals
        .push((acc_param.to_string(), ExcelValue::Empty));
    ctx.locals
        .push((val_param.to_string(), ExcelValue::Empty));
    let mut out = Vec::with_capacity(rows);
    let mut acc: Option<ExcelValue> = initial.cloned();
    let omit_first = initial.is_none();
    let mut first = true;
