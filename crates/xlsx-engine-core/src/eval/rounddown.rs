//! Excel `ROUNDDOWN(number, [num_digits])` — toward zero.
//!
...
//!
//! Desktop Excel / Microsoft ROUNDDOWN help:
//! - Always rounds **toward zero** (same direction as `TRUNC`; not `INT`).
//!   `ROUNDDOWN(-3.2, 0)` is `-3`; `ROUNDDOWN(-0.5, 0)` is `0`.
//! - `num_digits` omitted (one-arg form, or a trailing-comma / blank slot)
//!   is **0** — round to an integer. That matches `TRUNC(n)` and Microsoft's
//!   "if `num_digits` is 0" remark. `ROUNDUP` still requires two arguments.
...
//! - Arity: 1 or 2 arguments. `ROUNDDOWN()` / three-or-more is `#VALUE!`.
//!
...
//! 15-significant-digit snap keeps `ROUNDDOWN(1.15, 2)` at `1.15`. The naive
//! path issues two `powi` calls and has no specialised digits so benches
//! can print a before/after.
...
pub fn rounddown(n: f64, digits: i32) -> f64 {
    match digits {
        0 => toward_zero_int(n),
        1 => scale_down(n, 10.0, true),
        2 => scale_down(n, 100.0, true),
        3 => scale_down(n, 1_000.0, true),
        4 => scale_down(n, 10_000.0, true),
        -1 => scale_down(n, 10.0, false),
        -2 => scale_down(n, 100.0, false),
        -3 => scale_down(n, 1_000.0, false),
        -4 => scale_down(n, 10_000.0, false),
        d => excel_rounddown(n, d),
    }
}
...
/// Apply [`rounddown`] to every `(n[i], digits[i])`. Hot path for columns.
pub fn rounddown_slice(n: &[f64], digits: &[i32], out: &mut [f64]) {
    let len = n.len().min(digits.len()).min(out.len());
    for i in 0..len {
        out[i] = rounddown(n[i], digits[i]);
    }
}

/// Naive slice baseline matching [`rounddown_naive`].
pub fn rounddown_slice_naive(n: &[f64], digits: &[i32], out: &mut [f64]) {
    let len = n.len().min(digits.len()).min(out.len());
    for i in 0..len {
        out[i] = rounddown_naive(n[i], digits[i]);
    }
}

/// Broadcast one `num_digits` across a packed walk.
pub fn rounddown_slice_digits(n: &[f64], digits: i32, out: &mut [f64]) {
    let len = n.len().min(out.len());
    for i in 0..len {
        out[i] = rounddown(n[i], digits);
    }
}

/// Naive broadcast matching [`rounddown_naive`].
pub fn rounddown_slice_digits_naive(n: &[f64], digits: i32, out: &mut [f64]) {
    let len = n.len().min(out.len());
    for i in 0..len {
        out[i] = rounddown_naive(n[i], digits);
    }
}

/// `ROUNDDOWN(number, [num_digits])` — scalar context, omitted digits = 0.
pub(crate) fn fn_rounddown(
    ...
    Ok(ExcelValue::Number(rounddown(n, digits)))
}
...
#[inline]
fn scale_down(n: f64, p: f64, mul: bool) -> f64 {
    if n == 0.0 {
        return 0.0;
    }
    let sign = if n < 0.0 { -1.0 } else { 1.0 };
...
fn excel_rounddown(n: f64, digits: i32) -> f64 {
    ...
    let p = pow10_u(e);
    scale_down(n, p, digits > 0)
}

/// Snap binary leftovers that agree to Excel's 15 significant digits.
///
/// `1.15 * 100` is `114.999…` in IEEE; without a snap, `ROUNDDOWN` would
...
mod tests {
    ...
    fn microsoft_rounddown_examples() {
        assert_eq!(both(3.2, 0), 3.0);
        assert_eq!(both(76.9, 0), 76.0);
        assert_eq!(both(3.14159, 3), 3.141);
        assert_eq!(both(-3.14159, 1), -3.1);
        assert_eq!(both(31415.92654, -2), 31400.0);
    }

    #[test]
    fn toward_zero_not_int() {
        assert_eq!(both(-3.2, 0), -3.0);
    ...
    fn zero_and_nonfinite() {
        ...
        assert!(rounddown(f64::INFINITY, 0).is_infinite());
    }
    ...
    fn formula_microsoft_and_omitted_digits() {
        ...
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(3.2, 0)").unwrap(),
            ExcelValue::Number(3.0)
        );
        // Omitted num_digits defaults to 0 (TRUNC-aligned).
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(3.2)").unwrap(),
            ExcelValue::Number(3.0)
        );
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(-3.2,)").unwrap(),
        ...
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN()").unwrap(),
            ExcelValue::Error(ExcelError::Value)
        );
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(3.2, 0, 1)").unwrap(),
            ExcelValue::Error(ExcelError::Value)
        );
    }
    ...
    fn formula_errors_ltr_and_coerce() {
        let wb = Workbook::default();
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(#DIV/0!, #N/A)").unwrap(),
            ExcelValue::Error(ExcelError::Div0)
        );
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(\"76.9\", 0)").unwrap(),
            ExcelValue::Number(76.0)
        );
        assert_eq!(
            eval_formula_in(&wb, "=ROUNDDOWN(TRUE, -1)").unwrap(),
