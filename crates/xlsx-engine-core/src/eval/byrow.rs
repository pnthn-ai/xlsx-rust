...
use super::makearray::{fn_key, names_eq, resolve_lambda_arity, LambdaError, Local};
...
fn apply_general(
    ...
    ctx.locals.push(Local::provided(
        row_param.to_string(),
        ExcelValue::Array(vec![grid[0].clone()]),
    ));
    let mut out = Vec::with_capacity(grid.len());
    for row in grid {
        ctx.locals[base].value = ExcelValue::Array(vec![row.clone()]);
...
mod tests {
    ...
    fn classify_sum_and_index() {
        ...
        assert_eq!(
            classify(&parse("7").unwrap(), "r"),
            Some(RowPlan::Const(n(7.0)))
        );
