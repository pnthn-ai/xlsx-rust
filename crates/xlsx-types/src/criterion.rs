//! Excel `SUMIF` / `COUNTIF` / `SUMIFS`-style criteria.
//!
...
/// Compiled Excel criteria used by `SUMIFS` (and `SUMIF` / `COUNTIF`).
#[derive(Clone, Debug)]
...
impl Criterion {
    ...
    /// Whether `cell` satisfies this criteria (Excel SUMIF/COUNTIF/SUMIFS rules).
