...
- `TRUNC(number, [num_digits])`: toward zero (`TRUNC(-8.9)` is `-8`; that is not `INT(-8.9)` = `-9`, nor `ROUND(-1.5, 0)` = `-2`). Omitted `num_digits` (one-arg form, trailing-comma slot, or blank) is 0. Negative `num_digits` truncates left of the decimal (`31415.92654` / `-2` → `31400`). Arithmetic coerce; errors left-to-right; 0 or 3+ args is `#VALUE!`. A 15-digit snap keeps `TRUNC(1.15, 2)` at `1.15`. Kernel: [`eval/trunc.rs`](crates/xlsx-engine-core/src/eval/trunc.rs). `ROUND` / `ROUNDUP` / `ROUNDDOWN` / `INT` are separate.
