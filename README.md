...
| [`eval/fixed.rs`](crates/xlsx-engine-core/src/eval/fixed.rs) | Excel `FIXED` (ROUND + fixed-point text; default 2 decimals; optional `no_commas`) |
...
`FIXED(number, [decimals], [no_commas])` rounds with Excel `ROUND` and
returns **text**. Omitted `decimals` is 2; an empty slot is 0. Negative
`decimals` rounds left of the point. `no_commas` TRUE suppresses
thousands commas. Sign comes from the rounded value. `DOLLAR` / `TEXT`
/ `VALUE` are unchanged.
