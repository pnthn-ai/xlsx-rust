...
| [`eval/trim.rs`](crates/xlsx-engine-core/src/eval/trim.rs) | Excel `TRIM` (ASCII `0x20` collapse + end trim; tab/NBSP kept) |
...
- `TRIM(text)`: leading/trailing ASCII spaces (`0x20`) plus collapse of
  internal ASCII space runs to one space. Tab / CR / LF / NBSP (`CHAR(160)`)
  and other Unicode spaces are kept. Numbers / bools / blanks coerce like
