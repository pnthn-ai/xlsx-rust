...
| [`eval/upper.rs`](crates/xlsx-engine-core/src/eval/upper.rs) | Excel `UPPER` (SWAR ASCII + Unicode default; `ß` kept) |
...
- `UPPER(text)`: lowercase letters become uppercase. ASCII `a–z` → `A–Z`
  (invariant, not Turkish). Precomposed Latin / Greek / Cyrillic follow
  Unicode default (`é` → `É`). Sharp s `ß` is **not** converted to `SS`
  (Microsoft `UPPER` note). Numbers / bools / blanks coerce like `&`.
  Wrong arity is `#VALUE!`. `LOWER` is a later workstream.
