...
| [`eval/byrow.rs`](crates/xlsx-engine-core/src/eval/byrow.rs) | `BYROW(array, LAMBDA(row, body))` row-apply kernel (`SUM`/`MAX`/… specialized) |
...
- `MAKEARRAY(rows, cols, LAMBDA(r, c, body))`: 1-based indexes relative to
  ...
  `BYROW`) is `#CALC!` — this engine has no first-class function value.
- `BYROW(array, LAMBDA(row, body))`: applies a one-parameter LAMBDA to each
  row and returns an n×1 column. The bound `row` is a 1×w array. Wrong
  LAMBDA arity is `#VALUE!`; a missing LAMBDA is `#CALC!`. A body that
  returns an array is `#CALC!` in that result cell. Eta reduction
  (`BYROW(a, SUM)`) is supported for `SUM` / `AVERAGE` / `MIN` / `MAX` /
  `COUNT` / `COUNTA` / `COUNTBLANK` / `PRODUCT`. A body error stays in that
  cell. A scalar first argument is 1×1.
...
**`BYROW` / `LAMBDA` limits** (honest, not hidden behind a broken case):

- BYROW returns an array **value** (always n×1, including 1×1). The snippet
  workbook has no spill grid, so a blocked cell below the host never yields
  `#SPILL!`.
- Returning anything but a single value from the LAMBDA is `#CALC!` in that
  result cell — nested arrays (including `LAMBDA(r, r)`) are not stacked.
- Immediately-invoked `LAMBDA(...)(args)` is not parsed. Optional LAMBDA
  parameters and `LET` helpers are out of scope. Parameter names that
  tokenize as A1 refs are `#VALUE!`.
