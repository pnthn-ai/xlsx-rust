...
| [`dates.rs`](crates/xlsx-engine-core/src/dates.rs) | 1900/1904 serials, leap-year bug, `EDATE` / `EOMONTH` / `NETWORKDAYS` / `NETWORKDAYS.INTL` / `WEEKDAY` / `WEEKNUM` / `WORKDAY` / `WORKDAY.INTL` / `YEARFRAC` / `DAYS360` |
...
- 1900 leap-year bug (`DATE(1900,2,29)` is serial 60); 1904 date system.
  ...
  is Saturday can reach week 54 (System 1). `ISOWEEKNUM` is not this function.
